...
login: ## login to ghcr.io using a personal access token (PAT)
	@if [ -z "$(CR_PAT)" ]; then\
		...
		echo $(CR_PAT) | docker login ghcr.io -u florenciasilvestre --password-stdin;\
	fi

tag:
	@docker tag project:latest ghcr.io/florenciasilvestre/project:latest

push: tag ## docker push to container registry (ghcr.io)
	@docker push ghcr.io/florenciasilvestre/project:latest
